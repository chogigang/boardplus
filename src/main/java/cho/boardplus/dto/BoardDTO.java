...
public class BoardDTO {
    ...
    private List<Long> boardImgIds= new ArrayList<>(); //이미지 파일의 ID를 저장하는 List item 엔티티로 메핑될 때 사용될것 게시판 등록,수정 같이 이미지를 선택적으로 추가할 수 있는 경우 사용
    ...
    public static BoardDTO toBoardDTO(BoardEntity boardEntity) {
     ...
     //파일 첨부
       if(boardEntity.getFileAttached()==0){
            boardDTO.setFileAttached(boardEntity.getFileAttached());// 0 파일 첨부가 없을때
        }else{
